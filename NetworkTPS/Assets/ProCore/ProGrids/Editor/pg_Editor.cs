...
#region CONSTANT
	...
	#if PRO
	const int WINDOW_HEIGHT = 240; //EDIT by Gabe: subtracted 30px
	#else
	const int WINDOW_HEIGHT = 260; //EDIT by Gabe: subtracted 30px
	#endif
...
#region PREFERENCES
	...
	public void LoadPreferences()
	{
		...
		Shared.useAxisConstraints = useAxisConstraints;
...
#region MENU
	...
	{
		pc_AboutWindow.Init("Assets/ProCore/ProGrids/About/pc_AboutEntry_ProGrids.txt", true);
	}
	...
	public static void IncreaseGridSize()
	{
		...
		instance.SetSnapValue(instance.snapUnit, Mathf.Clamp( val, 0f, Mathf.Infinity));
		instance.Repaint();
	}
	...
	public void OnSceneGUI(SceneView scnview)
	{
		...
		{		
			if(lastTransform.position != lastPosition)
			{
				...
				if(constraintsOn)
				{
					// Bugger.SetKey("constraints", "on");
					selected.position = pg_Util.SnapValue(old, mask, snapValue);
				}
				else
				{
					// Bugger.SetKey("constraints", "off");
...
#region EVENT
	...
	{
		if(EditorApplication.currentScene != currentLevel)
		{
			currentLevel = EditorApplication.currentScene;
			pivot = Vector3.zero;
		}
	}
#endregion
...
#region GLOBAL SETTING
	...
	public void SetSharedSnapValues(bool enable, float snapVal)
	{
		...
		Shared.snapEnabled = enable;
		Shared.snapValue = snapVal;
	}
#endregion
}